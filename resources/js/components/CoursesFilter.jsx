...
const CoursesFilter = () => {
    ...
    useEffect(() => {
        if(urlParams.has('search')) setSearchQuery(urlParams.get('search'));
    }, [])

    function changeFilter(key, value){
        let nextRoute = `${window.location.pathname}?${key}=${value}`
        if(selectedTopic && key != 'topic') nextRoute += ('&topic='+selectedTopic)
        if(searchQuery != '' && key != 'search') nextRoute += ('&search='+searchQuery)
        router.visit(nextRoute)
    }

    function performSearch(){
        if(!searchQuery || searchQuery.trim() == '') return;
        changeFilter('search', searchQuery)
    }
        ...
        <div class="w-full md:w-full shadow p-5 rounded-lg bg-white mb-4">
            <div className="grid grid-cols-12 gap-2">
                <div class="relative col-span-8 md:col-span-10">
                    ...
                    <input onKeyPress={(e) => e.key === 'Enter' && performSearch()} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} type="text" placeholder="جست و جو ..." class="px-8 py-3 w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0 text-sm"/>
                </div>
